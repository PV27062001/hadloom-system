<?php
    ...
    if($_SESSION['login'])
    {
        ...
        if(isset($_POST['action'])){
            switch($_POST['action']){
                case "delete": {
                    $sql1="delete from employee where empid='".$_POST['id']."'";
                ...
                case "edit": {
                    ...
                    if(strtolower($g)=="female")
                        $g="F";
                    else if(strtolower($g)=="male")
                        $g="M";
                    else
                        $g="O";
                    $sql2="update employee set empname='".$_POST['name']."',gender='".$g."',address='".$_POST['address']."',phone='".$_POST['phone']."' where empid='".$_POST['id']."'";
...
<script type='application/javascript'>
    ...
    function editRow(row){
        ...
        let k = 1;
        while(k<5){
            sel.childNodes[k].innerHTML = "<input type='text' size='9' value='"+sel.childNodes[k].innerHTML+"'/>";
            k++;
        }
        for( key in data ){
            if(key == row)
    ...
    function editOK(row){
        ...
        var phone = sel.childNodes[2].firstChild.value;
        var address = sel.childNodes[3].firstChild.value;
        var gender = sel.childNodes[4].firstChild.value;
